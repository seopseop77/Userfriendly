...
Continuation (same session): operator requested live migration apply ("마이그레이션부터 적용해줘"). Migration 0018 applied via Supabase MCP `execute_sql` as one atomic `BEGIN ... COMMIT` block matching the 0013–0017 precedent. Pre-state confirmed alembic at `0017_drop_exchanges_session_id` and `participant_registrations` table absent. Apply ran CREATE TABLE + CREATE INDEX + alembic ledger bump in one transaction. Post-state across four axes all green: alembic ledger advanced to `0018_participant_registrations`; 9 columns match the migration intent exactly (`id uuid NOT NULL DEFAULT gen_random_uuid()`, `org_id uuid NOT NULL`, `token_hash text NOT NULL`, `name text NOT NULL`, `email text NOT NULL`, `institution text NOT NULL`, `research_description text NOT NULL`, `proposal_text text NULL`, `created_at timestamptz NOT NULL DEFAULT now()`); 4 constraints land (PK on `id`, FK `org_id → orgs(id)`, FK `token_hash → api_tokens(token_hash)`, UNIQUE `email` named `uq_participant_registrations_email`); 3 indexes land (`idx_participant_registrations_email` explicit btree, `participant_registrations_pkey` implicit PK btree, `uq_participant_registrations_email` implicit UNIQUE-backing btree — the two email indexes are intentional per §Decisions "redundancy is fine" rationale). **Live apply done; only remaining gate is operator-side Fly provisioning of the new `llm-tracker-signup` app (`fly apps create` + secrets + first `fly deploy`).** The signup app's `fly deploy` is independent of the still-pending proxy server `fly deploy` for migration 0017 from the prior cleanup track — separate apps, separate gates.

**Updated by**: Claude Code (signup app — code + live Supabase schema aligned; awaiting operator first `fly deploy` of llm-tracker-signup)
...
regressions); ruff clean. **Migration 0018 applied live to Supabase**
this same session via MCP `execute_sql` in one atomic `BEGIN ...
COMMIT` block matching the 0013–0017 precedent — alembic ledger
advanced `0017_drop_exchanges_session_id` → `0018_participant_registrations`;
post-state verified across columns / constraints / indexes (all green
per worklog §"Live apply timeline"). **Operator-owned remaining**:
(1) `fly apps create llm-tracker-signup` + `fly secrets set
LLMTRACK_DATABASE_URL=… LLMTRACK_PROXY_SERVER_URL=…` + `fly deploy
-c packages/llm_tracker_signup/fly.toml`; (2) smoke a test
registration end-to-end. The `[GITHUB_RELEASE_URL]` placeholder in
`success.html` is a separate follow-up (after the agent wheel is
published).
...
shipped that capability;
(2) `0033-plugin-analytics-no-rls.md` ADR
...
`AsyncEngine`;
(3) `content_level` made env-configurable (commit `2a68c56`) via
`LLMTRACK_CONTENT_LEVEL` (pydantic `Literal["L0", "L1", "L2", "L3"]`,
default `"L3"`) — replaces the hardcoded `"L3"` at three
`Exchange(...)` sites; required kwarg on the three helpers means a
forgotten forwarder site can't silently regress;
(4) i18n email scrubbing (commit `8b59887`) — `_EMAIL_RE` local-part
+ non-TLD-domain char classes swapped to `\w` (Python 3
...
**Migration 0018 applied live to Supabase** this same session via
MCP `execute_sql` (alembic ledger advanced `0017` → `0018`; table +
4 constraints + 3 indexes verified — see worklog §"Live apply
timeline"). **Operator-owned remaining**: (1) `fly apps create
llm-tracker-signup` + `fly secrets set LLMTRACK_DATABASE_URL=…
LLMTRACK_PROXY_SERVER_URL=…` + `fly deploy -c
packages/llm_tracker_signup/fly.toml`; (2) smoke a test registration
end-to-end and verify the row lands in `participant_registrations`.
The `[GITHUB_RELEASE_URL]` placeholder in `success.html` is a
separate follow-up after the agent wheel is published to GitHub
releases (Prompt 2 in the broader rollout).
...
and all 182 rows preserved (metadata-only column drop). Same
session also extended the track with `content_level` env-var
configurability (commit `2a68c56`) — `LLMTRACK_CONTENT_LEVEL`
plumbed from `Settings` → `app.state` → `forward_request` → all six
helper call sites; the three helpers now take `content_level: str`
as a required keyword-only argument so a forgotten site can't
silently regress to the prior hardcoded `"L3"`. Production default
unchanged (`LLMTRACK_CONTENT_LEVEL` not set in Fly secrets → still
`"L3"` on every row after redeploy). Same session also closed
i18n email scrubbing (commit `8b59887`) — `_EMAIL_RE` extended to
match Unicode local parts and raw IDN domains via Python 3's
