"""Auth surface (ADR-0020).

Axis 1 of ADR-0020: per-org bearer tokens. The agent presents
`Authorization: Bearer <token>`; the server hashes it (SHA-256 hex),
looks up `api_tokens`, resolves the org, and binds the per-request
DB session to the matching org for RLS (CP5).
